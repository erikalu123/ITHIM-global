...
prop <- io$bogota$trip_scen_sets |> 
  filter(participant_id !=0) |> 
  distinct(trip_id, scenario, .keep_all = T) |> 
  count(scenario, trip_mode) |> 
  mutate(freq = round(prop.table(n) * 100, 3), .by = scenario) |> 
  mutate(pd = round(freq - freq[scenario == 'baseline'], 1)) |> 
  filter(pd != 0)
...
print_pif_table <- function(mmets, censor_method = "WHO-QRL", plot_tile = ""){
  ...
  total_pif <- reframe(pif |> ungroup(), total = round(sum(value), 2), .by = name)
  print(ggplot(pif) +
          ...
          annotation_custom(tableGrob(total_pif, rows=NULL), 
                            xmin=0.015, xmax=0.020, ymin="pif_bus"))
...
print_pif_table(io$bogota$outcomes$mmets, censor_method = "WHO-DRL", plot_tile = "PIF for cycling/public transport with censor point at 17.5 MMET hours per week")
...
print_pif_table(io$bogota$outcomes$mmets, censor_method = "WHO-QRL", plot_tile = "PIF for cycling/public transport with censor point at 35 MMET hours per week")
...
print_pif_table(io$bogota$outcomes$mmets, censor_method = "none", plot_tile = "PIF for cycling/public transport with no censor point")
...
plotly::ggplotly(ggplot(df_deciles, aes(x = decile, y = value, color = Scenario)) +
  geom_line(aes(group = Scenario)) +
  geom_point() +
  labs(title = "Deciles of Value by Scenario", x = "Quantiles (0.1 - 0.9)", y = "MMET hours per week") +
...
```{r}
pif_deciles <- df_deciles |> pivot_wider(id_cols = decile, names_from = Scenario, values_from = value) |> mutate(across(3:5, ~ ifelse(. != 0, round(((Baseline - .)/.), 2), 0))) |> dplyr::select(-Baseline) |> pivot_longer(cols = -c(decile))

plotly::ggplotly(ggplot(pif_deciles) +
  aes(x = decile, y = value, fill = name) +
  geom_col() +
  scale_fill_hue(direction = 1) +
  theme_minimal())
