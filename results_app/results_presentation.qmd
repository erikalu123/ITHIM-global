...
total_trips <- io$bogota$trip_scen_sets |> filter(scenario == "baseline") |> 
...
```{r}
pif_deciles <- df_deciles |> pivot_wider(id_cols = decile, names_from = Scenario, values_from = value) |> mutate(across(3:5, ~ ifelse(. != 0, round(((Baseline - .)/.), 2), 0))) |> dplyr::select(-Baseline) |> pivot_longer(cols = -c(decile)) |> rename(Scenario = name)

y <- ggplot(pif_deciles) +
    aes(x = decile, y = value, fill = Scenario) +
    geom_col() +
    scale_fill_manual(values = scen_colours) +
    labs(x = "Deciles (0-0.9)", y = "Proportional difference from baseline", title = "")
    theme_minimal()
