...
for (city in cities) {
  ...
  td <- io[[city]]$trip_scen_sets |> filter(scenario == "baseline" & trip_id != 0 & trip_mode %in% c('pedestrian', 'cycle', 'car', 'motorcycle', 'bus')) |>  distinct(trip_id, .keep_all = T) |> count(trip_mode, trip_distance_cat) |> mutate(freq = prop.table(n), .by = trip_mode) |> dplyr::select(-n) |> dplyr::mutate(freq = round(freq * 100, 1)) |> pivot_wider(names_from = trip_distance_cat, values_from = freq) |> janitor::adorn_totals('col') |> mutate(Total = round(Total))
