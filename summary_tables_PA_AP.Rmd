...
for (x in 1:length(cities)) {
  ...
  summary <- as.data.frame(data_long %>% group_by(scenario) %>%
                             summarise('mean' = mean(pm_conc),
                                       ...
                                       '95th' = quantile(pm_conc, 0.9)) |> mutate_if(is.numeric, round, 2))
  
  # Extract scenario_pm with two columns with scenario names and scenario_pm_concentrations
  scenario_pm_df <- io[[cities[x]]]$outcomes$scenario_pm |> mutate_if(is.numeric, round, 2)
  ...
  summary <- left_join(summary, scenario_pm_df) |> mutate(change_PM = round(conc_pm - conc_pm[scenario == "Baseline"], 2))
...
co2_emission_inventory <- co2_emission_inventory |> mutate(change_baseline = !!rlang::sym(city) - bv, change_CO2_percentage = ifelse(change_baseline == 0, 0, round(change_baseline/bv * 100, 1)),
                                                           CO2_per_capita = round(!!rlang::sym(city)/pop_size, 2))
